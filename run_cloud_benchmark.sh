#!/bin/bash
# Script to run ETL pipeline in Docker with 10GB memory limit
# Simulates cloud environment constraints

set -e

echo "🚀 Starting Cloud Environment Simulation (10GB Memory Limit)"
echo "============================================================"
...
# Docker Compose prefixes network names with project name (directory name)
...
if [ -z "$NETWORK_NAME" ]; then
    ...
    # Try to find network by looking for etl_network_cloud in the name
...
# Run the ETL application with 10GB memory limit
echo ""
echo "🔵 Running ETL pipeline with 10GB memory limit..."
echo "   This simulates a cloud environment constraint"
echo ""
